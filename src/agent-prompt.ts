...
4. If dashbye.config.yml is missing, run "dashbye init --agent --json" with the known inputs above. When it returns "needs_input", surface that one question with the host's native menu UI when available, or ask one concise chat question. Call it again with accumulated flags until it returns "ready", show the preview, then run its writeCommand. Do not add browser profiles, cookies, credentials, or tokens.
...
9. Run dashbye validate, then connect only to an already running, manually authenticated dedicated Chrome and run dashbye inspect and dashbye plan. Do not open a login window automatically or control the user's daily Chrome.
