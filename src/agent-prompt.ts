...
9. Run dashbye validate, then launch official Chrome with a dedicated profile outside all repositories and the configured loopback remote-debugging endpoint. Request any required GUI permission instead of asking the owner to run the launch command. Never launch or control the owner's daily Chrome profile and never automate authentication. Ask the owner only to sign in to Google in the opened window. If the environment cannot launch GUI applications, explain the limitation and provide the exact fallback command. Then run dashbye inspect and dashbye plan; DashBye may navigate the dedicated session to the exact configured item.
